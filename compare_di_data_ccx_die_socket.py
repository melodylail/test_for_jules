#!/usr/bin/env python3
"""
Compare di_data across CCX, DIE, and SOCKET hierarchy levels.
...
OUTPUT_DIR = "results/visualizations/di_data_ccx_die_socket_comparison"
DATA_COLUMNS = ["IO DIE0", "IO DIE1", "IO DIE2"]
...
def load_data():
    """Load di_data_parsed.json for each dataset."""
    data = {}
    for ds in DATASETS:
        filepath = Path(DATA_DIR) / ds / "di_data_parsed.json"
...
def create_individual_chart(data, categories, column, output_dir):
    ...
    ax.set_title(f'di_data: {column} — CCX vs DIE vs SOCKET', fontsize=14)
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    outpath = Path(output_dir) / f"di_data_{col_safe}_comparison.png"
...
def create_dashboard(data, categories, output_dir):
    """Create a 1x3 dashboard with all 3 columns."""
    ...
    fig.suptitle('di_data Comparison: CCX vs DIE vs SOCKET',
                 fontsize=16, fontweight='bold', y=1.02)
    plt.tight_layout(rect=[0, 0, 1, 0.93])

    outpath = Path(output_dir) / "di_data_comparison_dashboard.png"
    fig.savefig(outpath, dpi=150)
    plt.close(fig)
    print(f"  Saved {outpath}")
...
def main():
    ...
    print("Loading di_data from CCX, DIE, and SOCKET...")
    data = load_data()
    if not data:
