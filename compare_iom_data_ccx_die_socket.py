#!/usr/bin/env python3
"""
Compare iom_data across CCX, DIE, and SOCKET hierarchy levels.
...
OUTPUT_DIR = "results/visualizations/iom_data_ccx_die_socket_comparison"
DATA_COLUMNS = ["CS0_RDSZ", "CS1_RDSZ", "CS2_RDSZ", "CS3_RDSZ",
                "CS0_WRSZ", "CS1_WRSZ", "CS2_WRSZ", "CS3_WRSZ",
                "TOTAL_BW_MIN", "TOTAL_BW_MAX"]


def parse_value(value_str):
    """Parse values with K/M/G suffixes and units like B/s, KB/s, MB/s, GB/s."""
...
def load_data():
    """Load iom_data_parsed.json for each dataset."""
    data = {}
    for ds in DATASETS:
        filepath = Path(DATA_DIR) / ds / "iom_data_parsed.json"
...
def create_individual_chart(data, categories, column, output_dir):
    ...
    ax.set_title(f'iom_data: {column} — CCX vs DIE vs SOCKET', fontsize=14)
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    outpath = Path(output_dir) / f"iom_data_{column}_comparison.png"
...
def create_dashboard(data, categories, output_dir):
    """Create a dashboard with all 10 columns (2 rows of 5)."""
    ...
    fig.suptitle('iom_data Comparison: CCX vs DIE vs SOCKET',
                 fontsize=16, fontweight='bold', y=1.0)
    plt.tight_layout(rect=[0, 0, 1, 0.96])

    outpath = Path(output_dir) / "iom_data_comparison_dashboard.png"
    fig.savefig(outpath, dpi=150)
    plt.close(fig)
    print(f"  Saved {outpath}")
...
def main():
    ...
    print("Loading iom_data from CCX, DIE, and SOCKET...")
    data = load_data()
    if not data:
