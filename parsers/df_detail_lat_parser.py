...
def parse_section(lines):
    header_line1_index = next((i for i, line in enumerate(lines) if 'DIE0' in line), -1)
    if header_line1_index == -1: return {}
    ...
    for i, start_pos in enumerate(col_starts):
        ...
        for die in die_headers:
            if die['start'] <= start_pos < die['end']: parent_die = die['name']; break
    ...
    for line in lines:
        ...
        if match:
            return {f"{match.group(1)}_{match.group(2)}": parsed_data}
